...
class Newsletter_Form_Builder {
	 ...
	 * Set properties.
	 */
    public function __construct($name,$id,$action,$method,$error){
        $this->name = $name;
        $this->id = $id;
        $this->action = $action;
        $this->method = $method;
    ...
    private function get_form_body(){
        ...
        $form .= '<input type="hidden" name="return_url" value="https://www.nationalarchives.gov.uk/notifications/newsletter-thank-you/">';
    ...
    private function get_paragraph() {
        return'<div class="form-row"><p>Subscribe now for regular news, updates and priority booking for events</p></div>';
    }
    ...
    private function get_legend(){
        return '<legend>Sign up to our mailing list</legend>';
    }
    ...
    private function get_email_input(){
        return '<div class="form-row"><label for="email">Email address</label><input type="email" name="email" id="email" placeholder="Enter your email address" aria-labelledby="newsletterAccessibility" required></div>';
    }
    ...
    private function get_first_name_input(){
        return '<div class="form-row"><label for="firstname">First name <span class="optional">(optional)</span></label><input type="text" name="firstname" id="firstname" placeholder="Enter your first name" aria-labelledby="newsletterAccessibility"></div>';
    }
    ...
    private function get_last_name_input(){
        return '<div class="form-row"><label for="lastname">Last name <span class="optional">(optional)</span></label><input type="text" name="lastname" id="lastname" placeholder="Enter your last name" aria-labelledby="newsletterAccessibility"></div>';
    }
    ...
    private function get_terms_checkbox(){
        return '<div class="form-row"><input id="privacy_policy" type="checkbox" name="privacy_policy"><label for="privacy_policy">I have read and understand the <a href="https://nationalarchives.gov.uk/legal/privacy-policy/">Privacy Policy</a>.</label>
    </div>';
    }
    ...
    private function is_sign_up() {
        return '<div class="form-row"><input id="newsletterSignUp" type="submit" name="Submit" value="Submit" class="margin-left-medium"></div>';
    }
    ...
    private function is_error_message() {
        if(isset($_POST['Submit'])){
            if(!empty($this->error)) {
                return '<div class="emphasis-block error-message">'. $this->error .'</div>';
            }
        }
    }
